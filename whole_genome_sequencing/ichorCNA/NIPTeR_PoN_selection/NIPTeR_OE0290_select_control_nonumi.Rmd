---
title: "select_control_nonumi"
author: "Pitithat Puranachot"
date: "12/8/2020"
output: html_document
---

This document shows how to perform Panel-of-Normal (PoN) selection for Picoplex lcWGS cfDNA samples. After selecting a sample dataset whose coverage are not so extreme from each others. The result file (e.g. CoverageQC_passed_nonumi_OE0290.tsv) of the previous step ( OE0290_ped_Picoplex_coverage_qc.html)  should be existed in the work directory of the R environment. If not, please find out where exactly the result file were saved.
...
control_bam_dir = 
  "/omics/groups/OE0436/internal/puranach/ctDNA/inform_liquidbiopsy/NIPTeR/control_samples_bams_OE0290_picoplex/"
```
...
# 
qc_passed_samples_df = read.delim("CoverageQC_passed_nonumi_OE0290.tsv",
...
for (row_num in seq(1:nrow(qc_passed_samples_df))) {
  ...
  sample_bam =
    paste0(sample_bam_dir,sample_id,"_merged.mdup.bam")
...
### please run create_nipt_object.R on every bam file as followed
# for bamfile in /abi/data2/puranach/ct_DNA/NIPTeR/control_samples_bams_OE0290_20201207/*callConsensus_realigned.bam; do cmd="module load R/3.6.0; Rscript /abi/data2/puranach/ct_DNA/NIPTeR/create_nipt_object.R ${bamfile}"; echo ${cmd} | bsub -J create_nipt_object_$(basename ${bamfile} .bam) -R "rusage[mem=150GB]" -q long; done
```
...
4. We will create NIPT object to be used as input of the following steps. Run create_nipt_object.R (in git repository) on every Symbolic-linked bam file. 
...
```
for bamfile in /omics/groups/OE0436/internal/puranach/ctDNA/inform_liquidbiopsy/NIPTeR/control_samples_bams_OE0290_picoplex/*_merged.mdup.bam; 
do 
...
nipt_obj_files = list.files(control_bam_dir,
                             pattern = "merged.mdup.rds$", full.names = T)
nipt_obj_list = list()
...
cat("Number of samples after NIPT filtering : ",
    length(niptControl_obj_list$samples))
```
...
NIPT_clean_sample_name = sapply(NIPT_control_group_separate_clean$samples, function(x){
  x$sample_name
})
NIPT_clean_sample_name = unname(NIPT_clean_sample_name)
NIPT_clean_sample_bamfilename=paste0(NIPT_clean_sample_name,".mdup.bam")
```
...
sample_id_split = as.data.frame(do.call(rbind,strsplit(bamfile_df$filename,split = "_")))

bamfile_df$PID = paste0(sample_id_split$V2,"_",sample_id_split$V3)            

bamfile_df = mutate(bamfile_df, 
                    fullpath = paste0(pid_dir,PID,"/alignment/",
                                      filename))

bamfile_df = mutate(bamfile_df,
                    if.exist=file.exists(fullpath))

### Check before if all files exist
