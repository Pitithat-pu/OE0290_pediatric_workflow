---
title: "OE0290_ped_AccelNGS_coverage_qc"
author: "Pitithat Puranachot"
date: "12/6/2020"
output: html_document
---

This document shows how to perform selection of sample to be used for Panel-of-Normal selection. Basically, this following script will read genomic coverage files (in stat_coverage/*_CollectWgsMetrics.txt) and extract MEAN_COVERAGE of every sample defined in the variable pids.
...
```{r reading in coverage files into dataframe}
...
for (pid in pids) {
  ...
  matrix_files = 
    list.files(pid_coverage_stat_dir, pattern = "_CollectWgsMetrics.txt",
               full.names = TRUE)
...
plot_coverage_distribution = function(coverage_stat_df){
  ...
  distribution_plot = ggplot(coverage_stat_df) + 
    ...
    labs(x="Coverage",y="Sample Frequency (%)",
         title = paste0("OE0290_pediatric: Coverage distribution (n=",
    ...
    geom_text(x= median(coverage_stat_df$MEAN_COVERAGE), y=0,
              label=paste0("med. ",
...
### Perform shapiro test for first 150 outliers.
...
for (delete_row in seq(nrow(coverage_df))) {
  ...
  shapiro_pvalue_list[[coverage_df$sample_id[delete_row]]] = 
    shapiro.test(temp_coverage_df$scaled_coverage)$p.value
...
### Find the first outlier that pass the shapiro p-value 0.05.
...
6. Plot the distribution of coverage after removing outlier samples
```{r create dataframe with normal distributed coverage samples}

norm_coverage_df = coverage_df %>% 
  filter(!sample_id %in% names(shapiro_pvalue_list)[1:row_number])
...
norm_coverage_df$pvalue = round(pnorm(norm_coverage_df$zscore),3)
norm_coverage_df = norm_coverage_df[order(norm_coverage_df$pvalue),]
...
plot_coverage_distribution(norm_coverage_df) +
  labs(title = paste("Normalized Coverage distribution (n=",
                       nrow(norm_coverage_df),")")) 
```
...
coverage_df$scaled_coverage = 
  (coverage_df$MEAN_COVERAGE - 
     mean(norm_coverage_df$MEAN_COVERAGE)) /
  sd(norm_coverage_df$MEAN_COVERAGE)
...
8. We convert the scaled coverage to probabilistic score and select samples with the score between 0.05 and 0.95.
...
filtered_samples = filter(coverage_df, pvalue < 0.05 | pvalue  >0.95)  %>% select(sample_id,MEAN_COVERAGE,pvalue)



after_filtered_coverage_df = filter(coverage_df, pvalue >= 0.05, 
                                    pvalue <=0.95)   %>% select(sample_id,MEAN_COVERAGE,pvalue)

test_normality = shapiro.test(after_filtered_coverage_df$MEAN_COVERAGE)
## get coverage cutoff ~ 0.2
  # test_normality$p.value
...
distribution_plot = ggplot(after_filtered_coverage_df) + 
    ...
    geom_vline(xintercept = 
                 mean(after_filtered_coverage_df$MEAN_COVERAGE)) + 
    geom_text(x= mean(after_filtered_coverage_df$MEAN_COVERAGE), y=0,
              label=
                paste0("avg.",round(mean(after_filtered_coverage_df$MEAN_COVERAGE),2)),
              vjust = 1)  + 
  geom_text(x=min(after_filtered_coverage_df$MEAN_COVERAGE), 
            y=0, label=paste0("min.",round(min(after_filtered_coverage_df$MEAN_COVERAGE),2)))  + 
  geom_text(x=max(after_filtered_coverage_df$MEAN_COVERAGE), 
            y=0, label=paste0("max.",round(max(after_filtered_coverage_df$MEAN_COVERAGE),2)))


# write.table(filtered_samples,"CoverageQC_fail_samples.tsv",sep="\t",row.names = FALSE)
distribution_plot

write.table(after_filtered_coverage_df,
            "CoverageQC_passed_OE0290.tsv",
            sep="\t",row.names = FALSE)

```
File CoverageQC_passed_OE0290.tsv will be used for selection of Panel-of-normal with NiPTER
